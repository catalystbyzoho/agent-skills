...
Written by `catalyst init` to the project root. If missing, `catalyst deploy` fails. The actual format is nested (NOT a flat `{project_id, env_id, ...}` object):
...
To read the active project: `defaults.project` gives the active `idx`; find the matching `projects[]` entry — its `id` is the project ID (`-p` value), and its `env[0].id` is the org ID (`--org` value). Note the org ID lives inside the `env` array, which is counterintuitive.
